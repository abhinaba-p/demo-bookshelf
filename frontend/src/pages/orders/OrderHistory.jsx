...
const OrderHistory = () => {
  ...
  useEffect(() => {
    const fetchAllOrdersProducts = async () => {
      ...
      if (!prevOrders || prevOrders.length === 0 || prevOrders == undefined || prevOrders.order.length == 0) {
        console.log('No Orders made from the customer yet!');
        return;
      }
      ...
      for (const order of prevOrders) {
        ...
        allProductsData[order.id] = products;
      }
  ...
  return (
    <>
      {isLoading ? (
        <p>Loading...</p>
      ) : prevOrders.length === 0 || prevOrders == undefined || prevOrders.order.length == 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-grow text-sm md:ml-4">
                        ...
                        <p className="mt-1">
                          Order Status:
                          <Chip size="sm" variant="outlined" color="green" value={order.payment_status == 'paid' ? 'confirm' : 'pending'} className="ml-1 rounded-full inline-flex items-center whitespace-nowrap p-1" />
                        </p>
                        <p className="mt-1">Order Date: {formattedDate}</p>
                        <p className="mt-1">Delivery Date: Not yet Shipped</p>
                        <p className="mt-1">Payment:
                          <Chip size="sm" variant="outlined" color="green" value={order.payment_status} className="ml-1 rounded-full inline-flex items-center whitespace-nowrap p-1" />
                        </p>
                        <p className="mt-1">Total Amount:
                          <Chip size="sm" variant="outlined" color="green" value={order.currency} className="ml-1 mr-1 rounded-full inline-flex items-center whitespace-nowrap p-1" />
                          {order.amount_total}
                        </p>
                      </div>
